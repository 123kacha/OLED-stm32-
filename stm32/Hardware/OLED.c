...
/**
  ...
  */
//void OLED_ShowImage(int16_t X, int16_t Y, uint8_t Width, uint8_t Height,uint8_t *Image)
//{
//	uint8_t i = 0, j = 0;
//	int16_t Page, Shift;
//	
...
{      	
	...
	for (i = 0; i < 16; i++)
	{
		OLED_WriteData(Chinese[i]);			//显示上半部分内容
	}
	OLED_SetCursor((Line - 1) * 2 + 1, (Column - 1) * 8);	//设置光标位置在下半部分
	for (i = 0; i < 16; i++)
	{
		OLED_WriteData(Chinese[i + 16]);		//显示下半部分内容
	}
}
